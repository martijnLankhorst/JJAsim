function out = JJAsim_2D_network_stationairyState(array,inputMode,IExt,f,n,z,th1,varargin)
%out = JJAsim_2D_network_stationairyState(array,inputMode,IExt,f,n,z,th1,varargin)
%
%DESCRIPTION
...
% - The problem is equivalent to solving the system:
%       M*cp(th) = IExtBase*IExt 
%       A*th = 2*pi*(z-f) 
%       A*pv(th) = 2*pi*(n-f)
%   where pv(x) = x-round(x/2/pi)*2*pi (principle value), A and M are matrices 
%   corresponding to Kirchhoff's current and voltage rule respectively, 
%   and cp(th) is the current-phase relation.
% - Requires an initial guess th1. This can be generated with the function call
%   JJAsim_2D_network_stationairyState_approximate.
...
%   (list)           Np* by W*      
% n (sweep)          Np* by Wf*     Vortex configuration
%   (list)           Np* by W*      
...
switch inputMode
    case 'sweep'       
        probFlag = 2; 
        W = prod([size(IExt,1),max([size(f,2),size(n,2),size(z,2)])]); 
    case 'list'        
        probFlag = 1; 
        W = max([size(IExt,1),size(f,2),size(n,2),size(z,2),size(th1,2)]);
    otherwise
...
[n,nCompact] = JJAsim_method_checkInput(n,'double',[Np,W],[1,probFlag],'n');
[z,zCompact] = JJAsim_method_checkInput(z,'double',[Np,W],[1,probFlag],'z');
if sum(sum(z - round(z) ~= 0)) > 1
   error('z must be integers') 
end
if strcmp(inputMode,'sweep') && size(f,2) > 1 && size(n,2) > 1 && size(f,2) ~= size(n,2)
   error('size of n inconsistent with size of f'); 
end
...
switch inputMode
    case 'sweep'
        sweepW = [size(IExt,1),max([size(f,2),size(n,2),size(z,2)])];
        ...
        nProblemList = JJAsim_method_sweep(size(n,2),2,sweepW,repetitions);
    ...
    case 'list'
        ...
        nProblemList = JJAsim_method_list(size(n,2),W,repetitions);
...
[nProblemSelection,nTableSelection,Wn] = JJAsim_method_partition(...
    nProblemList,computePartitions);
...
if parallelQ
    ...
    for P = 1:runs
        ...
        nP = JJAsim_method_tocell(n,nTableSelection(:,pInd),2,2);
        ...
        if ~array.inductanceQ 
            ...
            parfor p = coreInd
                [thOutTemp(:,:,p),IOutTemp(:,:,p),EOutTemp(:,p),solutionQTemp(:,p),...
                    nrOfStepsTemp(:,p)] = ...
                    ...
                    WIExt(p),fP{p},fCompact,fprobP(:,p),Wf(p),nP{p},nCompact,nprobP(:,p),...
                    Wn(p),zP{p},zCompact,zprobP(:,p),Wz(p),th1P{p},th1probP(:,p),Wth1(p),...
                    Ic,IcCompact,cpQ,cp,dcp,icp,storethQ,storeIQ,storeEQ,newton_steps,...
                    newton_tol);
            end
        else 
            ...
            parfor p = coreInd
                [thOutTemp(:,:,p),IOutTemp(:,:,p),EOutTemp(:,p),solutionQTemp(:,p),...
                    nrOfStepsTemp(:,p)] = ...
                    JJAsim_2D_network_stationairyState_priv_scr_CPU(Nis,Nj,Np,M,A,...
                    betaL,LMode,partitionSize,IExtBaseJ,pathArea,areaCompact,IExtP{p},...
                    IprobP(:,p),WIExt(p),fP{p},fCompact,fprobP(:,p),Wf(p),nP{p},...
                    nCompact,nprobP(:,p),Wn(p),zP{p},zCompact,zprobP(:,p),Wz(p),...
                    th1P{p},th1probP(:,p),Wth1(p),Ic,IcCompact,cpQ,cp,dcp,icp,...
                    storethQ,storeIQ,storeEQ,newton_steps,newton_tol);
            end
        end
    ...
    for p = 1:computePartitions
        ...
        nprob = nProblemSelection(:,p);       nsel = nTableSelection(:,p);
        ...
        if ~array.inductanceQ
            %solve initial condition for array without inductance
            [thp,Ip,Ep,solutionQ(:,p),nrOfSteps(:,p)] = ...
                JJAsim_2D_network_stationairyState_priv_noscr_CPU(Nis,Nj,Np,M,A,...
                partitionSize,IExtBaseJ,pathArea,areaCompact,IExt(Isel,:),Iprob,...
                WIExt(p),f(:,fsel),fCompact,fprob,Wf(p),n(:,nsel),nCompact,nprob,...
                Wn(p),z(:,zsel),zCompact,zprob,Wz(p),th1(:,th1sel),th1prob,Wth1(p),...
                Ic,IcCompact,cpQ,cp,dcp,icp,storethQ,storeIQ,storeEQ,newton_steps,...
                newton_tol);
        else
            %solve initial condition for array with inductance
            [thp,Ip,Ep,solutionQ(:,p),nrOfSteps(:,p)] = ...
                ...
                Iprob,WIExt(p),f(:,fsel),fCompact,fprob,Wf(p),n(:,nsel),nCompact,...
                nprob,Wn(p),z(:,zsel),zCompact,zprob,Wz(p),th1(:,th1sel),...
                th1prob,Wth1(p),Ic,IcCompact,cpQ,cp,dcp,icp,storethQ,storeIQ,storeEQ,...
