function array = JJAsim_2D_network_triangular(Nx,Ny,ax,ay,currentDirection,varargin)
%array = JJAsim_2D_network_triangular(Nx,Ny,ax,ay,direction,varargin)
%
%DESCRIPTION
% generates a Nx by Ny unit cells triangular array. A unit cell contains
% 2 islands. Inherits optional input from JJAsim_2D_network_create.
%
%FIXED INPUT
...
xnr = repmat((1:Nx)',1,Ny,2);
ynr = repmat((1:Ny),Nx,1,2);
incellnr = repmat(shiftdim(1:2,-1),Nx,Ny,1);
x = ((xnr-1) + shiftdim([0,0.5],-1))*ax;
y = (ynr-1 + shiftdim([0,0.5],-1))*ay*sqrt(3);
...
junctionIsland1 = jcs(:,3) + 2*(jcs(:,1)-1) + 2*Nx*(jcs(:,2)-1);
junctionIsland2 = jcs(:,6) + 2*(jcs(:,4)-1) + 2*Nx*(jcs(:,5)-1);

IExtBase = zeros(Nx,Ny,2);
switch currentDirection
    case 'x'
        IExtBase(xnr == 1 & incellnr == 1) = 1;
        IExtBase(xnr == Nx  & incellnr == 2) = -1;
    case 'y'
        IExtBase(ynr == 1  & incellnr == 1) = 1;
        IExtBase(ynr == Ny & incellnr == 2) = -1;
    otherwise
