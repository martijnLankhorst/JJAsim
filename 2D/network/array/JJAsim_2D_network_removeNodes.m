function [array,newNodeNrs,newJunctionNrs] = JJAsim_2D_network_removeNodes(array,nodeNrs)
%array = JJAsim_2D_network_removeIslands(array,nodeNrs)
%
%DESCRIPTION
...
% newNodeNrs        Nn by 1           Lists for all original nodes what new number is
%                                     assigned to it. 0 if removed.
% newJunctionNrs    Nj by 1           Lists for all original junctions what new number is
%                                     assigned to it. 0 if removed.
...
%remove nodes and junctions
nodes = true(array.Nn,1);
nodes(nodeNrs) = false;
cnodes = cumsum(nodes);
newNodeNrs = zeros(array.Nn,1);
newNodeNrs(nodes) = cnodes(nodes);
nodePosition = array.nodePosition(nodes,:);
j1 = array.junctionIsland1;
j2 = array.junctionIsland2;
ind = nodes(j1) & nodes(j2);
newJunctionNrs = zeros(array.Nj,1);
cjuncs = cumsum(ind);
newJunctionNrs(ind) = cjuncs(ind);
junctionIsland1 = cnodes(j1(ind));
junctionIsland2 = cnodes(j2(ind));
IExtBase = array.IExtBase(nodes);
Nn = sum(nodes);
