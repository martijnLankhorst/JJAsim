function [thOut,IOut,VOut,VtotOut,EOut] = ...
    JJAsim_2D_network_simulate_priv_scr_nocap_CPU(Nis,Nj,Np,M,A,LMode,betaL,W,...
    IExtBaseJ,IExtBaseTotal,pathArea,areaCompact,Nt,t,IExt,IExtCompact,IExtprob,WIExt,T,TCompact,Tprob,WT,f,...
    fCompact,fprob,Wf,z,zCompact,zprob,Wz,ic,icprob,Wic,Ic,IcCompact,cpQ,cp,icp,Rn,RnCompact,...
    storethQ,thTimePoints,Ntth,storeIQ,ITimePoints,NtI,storeVQ,VTimePoints,NtV,storeVtotQ,...
    VtotTimePoints,NtVtot,storeEQ,ETimePoints,NtE) %#ok<*INUSL>
...
% betaL (LMode = 1) Nj by Nj            junction-inductance matrix. sparse or dense.
...
thTimePoint = 1;        ITimePoint = 1;       VTimePoint = 1;     
VtotTimePoint = 1;      ETimePoint = 1;
...
%compute external current quantities
...
for i = 1:Nt-1
    ...
    yn = yn + randn(Nj,W).*sqrt(2*T(1,Tprob,iTemp)/dt./Rn);
    if LMode == 0
        thnp = AT*(D\(A*(g - thn + ILn)))/betaL;
    else
        thnp = AT*(DB\(A*(g - thn + ILn)));
    end
    thnp = thn - dt*Rn.*(yn-IEn-thnp);

    %store I, V and Vtot for this timestep
    if (storeIQ && ITimePoints(i)) || (storeVQ && VTimePoints(i)) || (storeVtotQ && VtotTimePoints(i)) || (storeEQ && ETimePoints(i))
        Vn = (thnp - thn)/dt;
    end
    ...
    %store I, V, Vtot and E for this timestep
    ...
    if storeEQ && ETimePoints(i)
        ...
        EOut(:,ETimePoint) = EOut(:,ETimePoint) + shiftdim(sum(In.*(betaL*In),1),1)/2;
