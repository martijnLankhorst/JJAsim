function [thOut,IOut,VOut,VtotOut,EOut] = JJAsim_2D_network_simulate_priv_noscr_cap_CPU(...
    Nis,Nj,Np,M,A,W,IExtBaseJ,IExtBaseTotal,pathArea,areaCompact,Nt,dt,IExt,...
    ...
    scheme,storethQ,thTimePoints,Ntth,storeIQ,ITimePoints,NtI,storeVQ,VTimePoints,NtV,storeVtotQ,...
...
%store instantaneous quantities at the first timestep
thm = ic1(:,ic1prob);
th0 = ic2(:,ic2prob);
...
if scheme == 0
    ...
    if storeEQ && ETimePoints(1)
        if cpQ
            EOut(:,1) = shiftdim(sum(icp(Ic,th0),1),1);
        else
            EOut(:,1) = shiftdim(sum(Ic.*(1-cos(th0)),1),1);
        end
        EOut(:,1) = EOut(:,1) + shiftdim(sum(betaC.*Vn.^2,1),1)/2;
        ETimePoint = 2;
    end
end
...
if scheme == 2
    ...
    if storeEQ && ETimePoints(Nt)
        if cpQ
            EOut(:,ETimePoint) = shiftdim(sum(icp(Ic,th2),1),1);
        else
            EOut(:,ETimePoint) = shiftdim(sum(Ic.*(1-cos(th2)),1),1);
        end
        EOut(:,ETimePoint) = EOut(:,ETimePoint) + shiftdim(sum(betaC.*V.^2,1),1)/2;
    end
end
end