...
% - This involves solving the following Langevin equation:
%       I = Ic*sin(th) + (1/Rn)*dth/dt + betaC*d^2th/dt^2 + eta
%       M*I = IExtBase*IExt 
%       A*th = 2*pi*(z-f*pathArea) - A*betaL*I 
%       corr_t(eta(t),eta(t+dt)) = 2 T/Rn delta(dt) 
%   where delta(dt) is the dirac-delta function. The A and M are matrices 
%   correspond to Kirchhoff's current and voltage rule respectively and
%   cp(th) is the current-phase relation. corr_t is the cross correlation.
...
%
%OUTPUT:
% out.th          	Nj by W*rep by Ntth	 gauge invariant phase difference output
% out.I          	Nj by W*rep by NtI 	 junction current output
% out.V          	Nj by W*rep by NtV 	 junction voltage drop output
% out.Vtot        	W*rep by NtVtot    	 total voltage drop output
% out.E          	W*rep by NtE       	 total energy output
        ...
        if ~array.inductanceQ && ~array.capacitanceQ
            parfor p = coreInd
                [thOutTemp(:,:,:,p),IOutTemp(:,:,:,p),VOutTemp(:,:,:,p),VtotOutTemp(:,:,p),EOutTemp(:,:,p)] = ...
                    ...
                    Nis,Nj,Np,M,A,partitionSize,IExtBaseJ,IExtBaseTotal,pathArea,areaCompact,Nt,t,IExtP{p},IExtCompact,...
        ...
        if array.inductanceQ && ~array.capacitanceQ
            parfor p = coreInd
                [thOutTemp(:,:,:,p),IOutTemp(:,:,:,p),VOutTemp(:,:,:,p),VtotOutTemp(:,:,p),EOutTemp(:,:,p)] = ...
                    ...
                    Nis,Nj,Np,M,A,LMode,betaL,partitionSize,IExtBaseJ,IExtBaseTotal,pathArea,areaCompact,Nt,t,IExtP{p},IExtCompact,...
                    IprobP(:,p),WIExt(p),TP{p},TCompact,TprobP(:,p),WT,fP{p},fCompact,fprobP(:,p),Wf(p),zP{p},...
                    zCompact,zprobP(:,p),Wz(p),th1P{p},th1probP(:,p),Wth1(p),Ic,IcCompact,cpQ,cp,icp,...
        ...
        if ~array.inductanceQ && array.capacitanceQ
            parfor p = coreInd
                [thOutTemp(:,:,:,p),IOutTemp(:,:,:,p),VOutTemp(:,:,:,p),VtotOutTemp(:,:,p),EOutTemp(:,:,p)] = ...
                    ...
                    Nis,Nj,Np,M,A,partitionSize,IExtBaseJ,IExtBaseTotal,pathArea,areaCompact,Nt,dt,IExtP{p},IExtCompact,IprobP(:,p),...
        ...
        if array.inductanceQ && array.capacitanceQ
            parfor p = coreInd
                [thOutTemp(:,:,:,p),IOutTemp(:,:,:,p),VOutTemp(:,:,:,p),VtotOutTemp(:,:,p),EOutTemp(:,:,p)] = ...
                    ...
                    Nis,Nj,Np,M,A,LMode,betaL,partitionSize,IExtBaseJ,IExtBaseTotal,pathArea,areaCompact,Nt,dt,IExtP{p},IExtCompact,...
                    IprobP(:,p),WIExt(p),TP{p},TCompact,TprobP(:,p),WT,fP{p},fCompact,fprobP(:,p),Wf(p),zP{p},...
    ...
    for p = 1:computePartitions
        ...
        if ~array.inductanceQ && ~array.capacitanceQ
            [thOut(:,:,:,p),IOut(:,:,:,p),VOut(:,:,:,p),VtotOut(:,:,p),EOut(:,:,p)] = ...
                JJAsim_2D_network_simulate_priv_noscr_nocap_CPU(...
                Nis,Nj,Np,M,A,partitionSize,IExtBaseJ,IExtBaseTotal,pathArea,areaCompact,Nt,t,IExt(Isel,:),IExtCompact,...
        ...
        if array.inductanceQ && ~array.capacitanceQ
            [thOut(:,:,:,p),IOut(:,:,:,p),VOut(:,:,:,p),VtotOut(:,:,p),EOut(:,:,p)] = ...
                JJAsim_2D_network_simulate_priv_scr_nocap_CPU(...
                Nis,Nj,Np,M,A,LMode,betaL,partitionSize,IExtBaseJ,IExtBaseTotal,pathArea,areaCompact,Nt,t,IExt(Isel,:),...
        ...
        if ~array.inductanceQ && array.capacitanceQ
            [thOut(:,:,:,p),IOut(:,:,:,p),VOut(:,:,:,p),VtotOut(:,:,p),EOut(:,:,p)] = ...
                JJAsim_2D_network_simulate_priv_noscr_cap_CPU(...
                Nis,Nj,Np,M,A,partitionSize,IExtBaseJ,IExtBaseTotal,pathArea,areaCompact,Nt,dt,IExt(Isel,:),IExtCompact,...
                Iprob,WIExt(p),T(Tsel,:),TCompact,Tprob,WT,f(:,fsel),fCompact,fprob,Wf(p),z(:,zsel),...
        ...
        if array.inductanceQ && array.capacitanceQ
            [thOut(:,:,:,p),IOut(:,:,:,p),VOut(:,:,:,p),VtotOut(:,:,p),EOut(:,:,p)] = ...
                JJAsim_2D_network_simulate_priv_scr_cap_CPU(...
                Nis,Nj,Np,M,A,LMode,betaL,partitionSize,IExtBaseJ,IExtBaseTotal,pathArea,areaCompact,Nt,dt,IExt(Isel,:),...
                IExtCompact,Iprob,WIExt(p),T(Tsel,:),TCompact,Tprob,WT,f(:,fsel),fCompact,fprob,Wf(p),...
